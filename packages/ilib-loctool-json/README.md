...
- outputSourceLocale: when `true`, write out a resource file for the source
  locale using the extracted source strings. This applies whether the json plugin
  writes localized json files directly or delegates output to a resource file
  type plugin configured in `resourceFileTypes`. Default is `false`.
...
## Delegating Output to Another Plugin (resourceFileTypes)

This plugin supports delegating output file writing to another plugin
configured via the `resourceFileTypes` setting. This is useful when you
want to use a different output format (such as JavaScript with custom
headers/footers) while still using this plugin's JSON parsing capabilities.

When a `resourceFileTypes` mapping exists for "json", the plugin will:
1. Extract strings from JSON files as normal
2. Instead of writing JSON files directly, delegate to the configured
   resource file type plugin for output

### Configuration

To delegate output to another plugin, add a `resourceFileTypes` mapping
...
In this example:
- JSON files are extracted using the `ilib-loctool-json` plugin with the
  specified schema
- Output files are written by `ilib-loctool-javascript-resource`, which
  adds the configured header and footer
- The output files are JavaScript modules instead of plain JSON

### How It Works

When localization occurs:
1. The plugin parses the source JSON and extracts localizable strings
2. Translations are applied to create localized JSON structures
3. If a `resourceFileTypes` mapping exists for "json", the localized
   content is converted to individual resources and passed to the
   configured resource file type plugin
4. The resource file type plugin handles the actual file writing with
   its own formatting

This approach allows combining the JSON parsing capabilities of this
plugin with the output formatting capabilities of other plugins.
...
This package is released under the [Apache License, Version 2.0](https://www.apache.org/licenses/LICENSE-2.0). The full license text is available in the [LICENSE](https://github.com/iLib-js/ilib-mono/blob/main/packages/ilib-loctool-json/LICENSE) file in the ilib-mono repository on GitHub.

## Release Notes

See [CHANGELOG.md](https://github.com/iLib-js/ilib-mono/blob/main/packages/ilib-loctool-json/CHANGELOG.md).
