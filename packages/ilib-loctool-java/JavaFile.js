...
JavaFile.prototype.makeKey = function(source) {
    return this.API.utils.hashKey(JavaFile.cleanString(source));
};
    ...
    while (result && result.length > 1 && result[2]) {
        ...
        if (result[2] && result[2].length) {
            ...
            var r = this.API.newResource({
                ...
                source: this.API.utils.trimEscaped(JavaFile.unescapeString(result[2])),
                autoKey: true,
