...
JsonFile.unescapeString = function (string) {
    ...
    unescaped = unescaped.replace(/^\\\\/g, "\\").replace(/([^\\])\\\\/g, "$1\\").replace(/\\(.)/g, "$1");
...
JsonFile.unescapeProp = function (prop) {
    return prop.replace(/~1/g, "/").replace(/~0/g, "~");
};

JsonFile.escapeRef = function (prop) {
    return JsonFile.escapeProp(prop).replace(/%/g, "%25").replace(/\^/g, "%5E").replace(/\|/g, "%7C").replace(/\\/g, "%5C").replace(/"/g, "%22").replace(/ /g, "%20");
};

JsonFile.unescapeRef = function (prop) {
    return JsonFile.unescapeProp(prop.replace(/%5E/g, "^").replace(/%7C/g, "|").replace(/%5C/g, "\\").replace(/%22/g, "\"").replace(/%20/g, " ").replace(/%25/g, "%"));
};
...
JsonFile.prototype.extractFromPrimitive = function (localizable, json, ref, translations, locale, returnValue, type) {
    ...
    switch (localizable) {
        case true:
        case "source":
            const __ret = this.handleSource(json, ref, translations, locale, returnValue, type);
            var text = __ret.text;
