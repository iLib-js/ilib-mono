 ...
 */
var webOSXliffIntermediateFile = function(options) {
    this.path = options.path;
    this.xliff = XliffFactory(options);
};

webOSXliffIntermediateFile.prototype = new IntermediateFile();
webOSXliffIntermediateFile.prototype.constructor = webOSXliffIntermediateFile;
webOSXliffIntermediateFile.prototype.parent = IntermediateFile.prototype;

/**
 * @override
 */
webOSXliffIntermediateFile.prototype.read = function() {
    if (!fs.existsSync(this.path)) {
        throw new Error("File not found: " + this.path);
    }
    var data = fs.readFileSync(this.path, "utf-8");
    var tuSet = this.xliff.deserialize(data);   
    return tuSet;
};
...
module.exports = webOSXliffIntermediateFile;