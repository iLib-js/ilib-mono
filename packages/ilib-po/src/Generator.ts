...
class Generator {
  ...
  constructor(options: GeneratorOptions) {
    if (!options)
      throw new Error(
        "Generator: Missing required options in Generator constructor",
      );
    ...
    this.plurals =
      pluralForms[this.targetLocale.getLanguage() ?? "en"] || pluralForms.en;
  }
  ...
  generate(set: TranslationSet): string {
    ...
    for (let i = 0; i < resources.length; i++) {
      ...
      const key: string =
        r.getKey() ??
        makeKey(type, r.getSource(), this.contextInKey && r.getContext());
      output += "\n";
   ...
   */
  private getTranslationOrEmptyString(
    translatedPlurals: Plural,
    sourcePlurals: Plural,
