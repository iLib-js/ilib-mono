...
AndroidResourceFile.prototype.parse = function(data) {
    ...
    if (resources.string) {
        ...
        for (var i = 0; i < strArr.length; i++) {
            ...
            if (utils.isAndroidResource(strArr[i]._text)) {
                // note that this is a used resource?
                logger.trace("Already resourcified");
            } else {
                var params = {
                    key: strArr[i]._attributes.name,
                    pathName: this.pathName,
                    context: this.context || undefined,
                    project: this.project.getProjectId(),
                    comment: strArr[i]._attributes.i18n,
                    dnt: utils.isDNT(strArr[i]._attributes.i18n),
    ...
    if (resources["string-array"]) {
        ...
        for (var i = 0; i < arrays.length; i++) {
            strArr = arrays[i];
            if (!strArr.item || strArr.item.every(function(item) { return utils.isAndroidResource(item._text);})) {
                // note that this is a used resource?
                logger.trace("Already resourcified");
            } else {
                var params = {
                    key: strArr._attributes.name,
                    pathName: this.pathName,
                    context: this.context || undefined,
                    project: this.project.getProjectId(),
                    subtype: "string-array",
                    comment: strArr._attributes.i18n,
                    dnt: utils.isDNT(strArr._attributes.i18n),
                    datatype: this.type.datatype,
                    flavor: this.flavor,
                    state: "new",
                    index: this.resourceIndex++
                ...
                if (locale === this.project.sourceLocale) {
                    params.sourceArray = strArr.item.map(function(item) {
                        return item._text;
                    });
                    params.sourceLocale = locale;
                } else {
                    params.sourceLocale = this.project.sourceLocale;
                    params.targetArray = strArr.item.map(function(item) {
                        return item._text;
                    });
                    params.targetLocale = locale;
                }
        ...
        for (var i = 0; i < plurals.length; i++) {
            ...
            if (strArr && strArr.item && ilib.isArray(strArr.item)) {
                strArr.item.forEach(function(item) {
                    if (typeof(item._text) === "undefined") {
                        logger.error("Syntax error in file " + this.pathName + ", item " + strArr._attributes.name + " quantity " + item._attributes.quantity + ". Probably have unescaped XML elements in this string.");
                    } else {
                        items[item._attributes.quantity] = item._text.trim();
                    }
                }.bind(this));
                var params = {
                    key: strArr._attributes.name,
            ...
            if (resource instanceof ContextResourceString || resource instanceof ResourceString) {
                if (resource.getTarget() || resource.getSource()) {
                    ...
                    if (typeof(resource.formatted) !== "undefined") {
                        entry._attributes.formatted = resource.formatted;
                    }
                    entry["_text"] = escapeString(target);
                ...
                for (var quantity in items) {
                    if (items[quantity]) {
                        arr.item.push({
                            ...
                            "_text": escapeString(items[quantity])
                        });
                    }
                }
