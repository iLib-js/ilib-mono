...
describe("testResourceGNUPrintfMatch", () => {
      ...
      test("should report both missing and extra parameters", () => {
          ...
          expect(Array.isArray(actual)).toBeTruthy();
          expect(actual && Array.isArray(actual) && actual.length).toBe(1);
          const description = Array.isArray(actual) ? actual[0].description : '';
          expect(description).toContain("Source string GNU printf parameter %2$d not found in the target string.");
          expect(description).toContain("Extra target string GNU printf parameter %1$f not found in the source string.");
      });
      ...
      test("should report mismatched parameters in array resources", () => {
          ...
          if (result) {
              expect(result.description).toContain("Source string GNU printf parameter %2$d not found in the target string.");
              expect(result.description).toContain("Extra target string GNU printf parameter %2$s not found in the source string.");
              expect(result.highlight).toContain('<e0>'); // At least one highlight
          }
      });
      ...
      test("should report mismatched parameters in plural resources", () => {
          ...
          const result = actual && Array.isArray(actual) && actual[0];
          if (result) {
              expect(result.description).toContain("Source string GNU printf parameter %1$s not found in the target string.");
              expect(result.description).toContain("Extra target string GNU printf parameter %1$d not found in the source string.");
              expect(result.highlight).toContain('<e0>'); // At least one highlight
          }
      });
