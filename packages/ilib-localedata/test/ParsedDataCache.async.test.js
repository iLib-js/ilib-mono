...
describe('ParsedDataCache Async Tests (Node and Browser)', () => {
    ...
    describe('getParsedData', () => {
        describe('Load data from .js files asynchronously', () => {
            test('should load and cache data from .js files asynchronously', async () => {
                ...
                const roots = ['./test/testfiles/files3'];
                const basename = 'info';

                const result = await parsedDataCache.getParsedData(locale, roots, basename);

                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', 'en-US')).toBe(false);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', 'en')).toBe(true);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', null)).toBe(true);
            ...
            test('should load and cache data from .js files with only root data asynchronously', async () => {
                ...
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'datefmt', 'en-GB')).toBe(false);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'datefmt', 'en')).toBe(false);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'datefmt', null)).toBe(true);
                expect(parsedDataCache.getCachedEntryCount()).toBeGreaterThan(0);
            ...
            test('should load and cache data from .js files with only root data asynchronously', async () => {
                ...
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'address', 'en-GB')).toBe(false);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'address', 'en')).toBe(true);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'address', null)).toBe(true);
                expect(parsedDataCache.getCachedEntryCount()).toBeGreaterThan(0);
                // ParsedDataCache should return undefined for en-GB since it doesn't exist in the file
                // The file only has root and en data, not en-GB data
                expect(result).toBeUndefined();
            ...
            test('should load data from .js file and return it from the cache', async () => {
                ...
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'address', 'en-AU')).toBe(true);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'address', 'en')).toBe(true);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'address', null)).toBe(true);
                expect(parsedDataCache.getCachedEntryCount()).toBeGreaterThan(0);
            ...
            test("should return nothing if the locale's .js file is not found", async () => {
                ...
                const result = await parsedDataCache.getParsedData(locale, roots, basename);
                expect(result).toBeUndefined();
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'address', 'kk-KZ')).toBe(false);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'address', 'kk')).toBe(false);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'address', null)).toBe(false);
            ...
            test("should return the cached sublocale data on the second call", async () => {
                ...
                const result = await parsedDataCache.getParsedData(locale, roots, basename);
                expect(result).toBeUndefined();
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'foo', 'en-US')).toBe(false);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'foo', 'en')).toBe(true);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'foo', null)).toBe(true);
                expect(parsedDataCache.getCachedEntryCount()).toBeGreaterThan(0);
        ...
        describe('Load data from flat .json files asynchronously', () => {
            test('should load data from flat .json files asynchronously', async () => {
                ...
                const roots = ['./test/testfiles/files4'];
                const basename = 'info';
                ...
                expect(parsedDataCache.hasParsedData('./test/testfiles/files4', 'info', 'zh-Hant-TW')).toBe(true);
                expect(parsedDataCache.getCachedEntryCount()).toBeGreaterThan(0);
            });
            ...
            test('should fall back from .js files to .json files in another root when .js files have no data for locale', async () => {
                ...
                const roots = ['./test/testfiles/files3', './test/testfiles/files4'];
                const basename = 'info';
            ...
            test('should fall back to .json files for different basenames', async () => {
                ...
                expect(parsedDataCache.hasParsedData('./test/testfiles/files4', 'bar', 'de-DE')).toBe(true);
                expect(parsedDataCache.getCachedEntryCount()).toBeGreaterThan(0);
            });
        });
        ...
        describe('Load data from hierarchical .json files asynchronously', () => {
            test('should load and cache data from hierarchical .json files asynchronously', async () => {
                ...
                const locale = new Locale('en-US');
                const roots = ['./test/testfiles/files2'];
                ...
                expect(parsedDataCache.hasParsedData('./test/testfiles/files2', 'merge', 'en-US')).toBe(true);
                expect(parsedDataCache.getCachedEntryCount()).toBeGreaterThan(0);
            });
            ...
            test('should load all sublocales of a hierarchical .json file asynchronously', async () => {
                ...
                expect(parsedDataCache.hasParsedData('./test/testfiles/files2', 'merge', 'en-US')).toBe(true);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files2', 'merge', 'en')).toBe(true);
                expect(parsedDataCache.hasParsedData('./test/testfiles/files2', 'merge', null)).toBe(false);
            });
        });
        ...
        describe('other tests', () => {
            test('should return undefined for invalid file content', async () => {
                ...
                const locale = new Locale('en-US');
                const roots = ['./test/testfiles/files5'];
                const basename = 'info';
            ...
            test('should handle multiple roots and return data from the first available', async () => {
                ...
                expect(parsedDataCache.hasParsedData('./test/testfiles/files4', 'info', 'en-US')).toBe(true);
                expect(parsedDataCache.getCachedEntryCount()).toBeGreaterThan(0);
            ...
            test('should return data from first available root', async () => {
                ...
                const locale = new Locale('en-US');
                const roots = ['./test/testfiles/files4', './test/testfiles/files5'];
                const basename = 'info';
    ...
    describe('hasParsedData', () => {
        ...
        test('should return false for non-existent parsed data', () => {
            expect.assertions(1);
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'nonexistent', 'en-US')).toBe(false);
        });
    ...
    describe('storeParsedData', () => {
        test('should store parsed data correctly', () => {
            ...
            parsedDataCache.storeParsedData('./test/testfiles/files3', 'test', 'en-US', data);

            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'test', 'en-US')).toBe(true);
            expect(parsedDataCache.getCachedData('./test/testfiles/files3', 'test', 'en-US')).toBe(data);
        });
        ...
        test('should overwrite existing data', () => {
            ...
            parsedDataCache.storeParsedData('./test/testfiles/files3', 'test', 'en-US', data1);
            parsedDataCache.storeParsedData('./test/testfiles/files3', 'test', 'en-US', data2);

            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'test', 'en-US')).toBe(true);
            expect(parsedDataCache.getCachedData('./test/testfiles/files3', 'test', 'en-US')).toBe(data2);
        });
    });
    ...
    describe('removeParsedData', () => {
        test('should remove existing parsed data', () => {
            ...
            parsedDataCache.removeParsedData('./test/testfiles/files3', 'test', 'en-US');
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'test', 'en-US')).toBe(false);
        });

        test('should handle removing non-existent data gracefully', () => {
            expect.assertions(1);
            parsedDataCache.removeParsedData('./test/testfiles/files3', 'nonexistent', 'en-US');
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'nonexistent', 'en-US')).toBe(false);
        });
    });
    ...
    describe('clearAllParsedData', () => {
        test('should clear all parsed data from the cache', () => {
            ...
            parsedDataCache.storeParsedData('./test/testfiles/files3', 'test1', 'en-US', { test: 'data1' });
            parsedDataCache.storeParsedData('./test/testfiles/files3', 'test2', 'en-GB', { test: 'data2' });
            ...
            expect(parsedDataCache.getCachedEntryCount()).toBe(0);
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'test1', 'en-US')).toBe(false);
        });
    });
    ...
    describe('getCachedEntryCount', () => {
        ...
        test('should return correct count for populated cache', () => {
            ...
            const initialCount = parsedDataCache.getCachedEntryCount();
            parsedDataCache.storeParsedData('./test/testfiles/files3', 'test', 'en-US', { test: 'data' });
    ...
    describe('Integration with DataCache', () => {
        ...
        test('should persist data across ParsedDataCache instances', () => {
            ...
            const newParsedDataCache = new ParsedDataCache(loader);
            expect(newParsedDataCache.hasParsedData('./test/testfiles/files3', 'test', 'en-US')).toBe(true);
            expect(newParsedDataCache.getCachedData('./test/testfiles/files3', 'test', 'en-US')).toBe(data);
        });
    });
    ...
    describe('storeData', () => {
        test('should parse and store function-based data correctly', () => {
            ...
            parsedDataCache.storeData(functionData, './test/testfiles/files3');

            // The storeData method stores data with basename "memory.js"
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'memory.js', null)).toBe(true);
            expect(parsedDataCache.getCachedData('./test/testfiles/files3', 'memory.js', null)).toEqual({ a: 'b', c: 'd' });
        });
        ...
        test('should parse and store object-based data with default property correctly', () => {
            ...
            parsedDataCache.storeData(objectData, './test/testfiles/files3');
        ...
        test('should parse and store string-based data correctly', () => {
            ...
            parsedDataCache.storeData(stringData, './test/testfiles/files3');
        ...
        test('should handle invalid data gracefully', () => {
            expect.assertions(1);
            const initialCount = parsedDataCache.getCachedEntryCount();
            parsedDataCache.storeData(null, './test/testfiles/files3');
    ...
    describe('Parsing Correctness', () => {
        test('should correctly parse .js file data and store by locale and basename', async () => {
            ...
            const rootInfo = parsedDataCache.getCachedData('./test/testfiles/files3', 'info', null);
            const rootFoo = parsedDataCache.getCachedData('./test/testfiles/files3', 'foo', null);
            expect(rootInfo).toEqual({"a": "b", "c": "d"});
            expect(rootFoo).toEqual({"m": "n", "o": "p"});

            // Check that en data is stored
            const enInfo = parsedDataCache.getCachedData('./test/testfiles/files3', 'info', 'en');
            const enFoo = parsedDataCache.getCachedData('./test/testfiles/files3', 'foo', 'en');
        ...
        test('should correctly parse .json file data and store by locale and basename', async () => {
            ...
            expect(parsedDataCache.hasParsedData('./test/testfiles/files4', 'info', 'en')).toBe(false);

            // Check that root data is not stored (this file exists but it is not loaded yet)
            const rootData = parsedDataCache.getCachedData('./test/testfiles/files4', 'info', null);
        ...
        test('should correctly parse multi-locale .json file data', async () => {
            ...
            expect(parsedDataCache.hasParsedData('./test/testfiles/files4', 'info', 'multi')).toBe(false);
            expect(parsedDataCache.getCachedEntryCount()).toBe(0);
    ...
    describe('Cache Behavior', () => {
        ...
        test('should cache data by locale and basename correctly', async () => {
            ...
            const enUSInfo = parsedDataCache.getCachedData('./test/testfiles/files3', 'info', 'en-US');
        ...
        test('should handle mixed cache hits and misses correctly', async () => {
            ...
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', 'en-US')).toBe(false);
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', 'fr-FR')).toBe(false);
        });
    ...
    describe('Parameter Validation', () => {
        test('should handle invalid parameters in getParsedData', async () => {
            ...
            const nullLocaleResult = await parsedDataCache.getParsedData(null, ['./test/testfiles/files3'], 'info');
            expect(nullLocaleResult).toBeDefined();

            // Test with undefined locale
            const undefinedLocaleResult = await parsedDataCache.getParsedData(undefined, ['./test/testfiles/files3'], 'info');
            ...
            const nullBasenameResult = await parsedDataCache.getParsedData(new Locale('en-US'), ['./test/testfiles/files3'], null);
            expect(nullBasenameResult).toBeUndefined();

            // Test with undefined basename
            const undefinedBasenameResult = await parsedDataCache.getParsedData(new Locale('en-US'), ['./test/testfiles/files3'], undefined);
            expect(undefinedBasenameResult).toBeUndefined();
        });

        // NOTE: getParsedDataSync tests moved to ParsedDataCache.sync.test.js
        // because they require synchronous file loading which is not available in browser
        ...
        test('should handle invalid parameters in hasParsedData', () => {
            ...
            // Test with null basename
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', null, 'en-US')).toBe(false);

            // Test with undefined basename
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', undefined, 'en-US')).toBe(false);

            // Test with empty string root
            expect(parsedDataCache.hasParsedData('', 'info', 'en-US')).toBe(false);

            // Test with empty string basename
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', '', 'en-US')).toBe(false);
        });
        ...
        test('should handle invalid parameters in getCachedData', () => {
            ...
            // Test with null basename
            const nullBasenameResult = parsedDataCache.getCachedData('./test/testfiles/files3', null, 'en-US');
            expect(nullBasenameResult).toBeUndefined();

            // Test with undefined basename
            const undefinedBasenameResult = parsedDataCache.getCachedData('./test/testfiles/files3', undefined, 'en-US');
            expect(undefinedBasenameResult).toBeUndefined();
            ...
            // Test with empty string basename
            const emptyBasenameResult = parsedDataCache.getCachedData('./test/testfiles/files3', '', 'en-US');
    ...
    describe('Edge Cases and Error Handling', () => {
        test('should handle invalid JSON data gracefully (line 219)', async () => {
            ...
            // Use zz-ZZ locale which has intentionally malformed JSON content
            // The file is in testfiles/files (not files3) to avoid webpack parse errors
            const locale = new Locale('zz-ZZ');
            const roots = ['./test/testfiles/files'];
            const basename = 'info';
        ...
        // NOTE: ESM module extension test moved to ParsedDataCache.sync.test.js
        // because _getJsFileExtensions calls _getModuleTypeSync which requires sync file loading
        ...
        test('should handle invalid parameters in storeParsedData (lines 583, 587)', () => {
            ...
            parsedDataCache.storeParsedData('./test/testfiles/files3', undefined, 'en-US', { test: 'data' });
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', undefined, 'en-US')).toBe(false);

            // Test with null locale (line 587)
            parsedDataCache.storeParsedData('./test/testfiles/files3', 'info', null, { test: 'root data' });
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', null)).toBe(true);
        ...
        test('should handle invalid parameters in removeParsedData (lines 601, 605)', () => {
            ...
            parsedDataCache.storeParsedData('./test/testfiles/files3', 'info', 'en-US', { test: 'data' });
            parsedDataCache.storeParsedData('./test/testfiles/files3', 'info', null, { test: 'root data' });

            // Test with null root (line 601)
            parsedDataCache.removeParsedData(null, 'info', 'en-US');
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', 'en-US')).toBe(true); // Should still exist

            // Test with undefined basename (line 601)
            parsedDataCache.removeParsedData('./test/testfiles/files3', undefined, 'en-US');
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', 'en-US')).toBe(true); // Should still exist

            // Test with null locale (line 605) - should remove root data
            parsedDataCache.removeParsedData('./test/testfiles/files3', 'info', null);
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', null)).toBe(false);
    ...
    describe('Multi-root data loading', () => {
        test('should load data from all roots for a given locale', async () => {
            ...
            const result = await parsedDataCache.getParsedData('de-DE', ['./test/testfiles/files3', './test/testfiles/files4'], 'foo');

            expect(result).toBeDefined();

            // Verify data was loaded from files3
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'foo', 'de-DE')).toBe(true);
            const files3Data = parsedDataCache.getCachedData('./test/testfiles/files3', 'foo', 'de-DE');
            ...
            expect(parsedDataCache.hasParsedData('./test/testfiles/files4', 'foo', 'de-DE')).toBe(true);
            const files4Data = parsedDataCache.getCachedData('./test/testfiles/files4', 'foo', 'de-DE');
        ...
        test('should load data from all roots even when some roots have no data', async () => {
            ...
            const result = await parsedDataCache.getParsedData('en-US', ['./test/testfiles/files3', './test/testfiles/files5'], 'info');
            ...
            expect(parsedDataCache.hasParsedData('./test/testfiles/files5', 'info', 'en-US')).toBe(false);
        });
        ...
        test('should load root data from all roots', async () => {
            ...
            const result = await parsedDataCache.getParsedData(null, ['./test/testfiles/files3', './test/testfiles/files4'], 'info');

            expect(result).toBeDefined();

            // Verify root data was loaded from files3
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', null)).toBe(true);
            const files3RootData = parsedDataCache.getCachedData('./test/testfiles/files3', 'info', null);
            ...
            expect(parsedDataCache.hasParsedData('./test/testfiles/files4', 'info', null)).toBe(true);
            const files4RootData = parsedDataCache.getCachedData('./test/testfiles/files4', 'info', null);
        ...
        test('should cache null for missing data in all roots', async () => {
            ...
            const result = await parsedDataCache.getParsedData('non-existent', ['./test/testfiles/files3', './test/testfiles/files4'], 'non-existent');

            expect(result).toBeUndefined();

            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'non-existent', 'non-existent')).toBe(false);
            expect(parsedDataCache.hasParsedData('./test/testfiles/files4', 'non-existent', 'non-existent')).toBe(false);
        });
        ...
        test('should load different data formats from different roots', async () => {
            ...
            const result = await parsedDataCache.getParsedData('en-US', ['./test/testfiles/files3', './test/testfiles/files4'], 'info');
            ...
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', 'en')).toBe(true);
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', null)).toBe(true);
            const jsData = parsedDataCache.getCachedData('./test/testfiles/files3', 'info', 'en');
            ...
            expect(parsedDataCache.hasParsedData('./test/testfiles/files4', 'info', 'en-US')).toBe(true);
            const jsonData = parsedDataCache.getCachedData('./test/testfiles/files4', 'info', 'en-US');
        ...
        test('should maintain separate cache entries for each root', async () => {
            ...
            await parsedDataCache.getParsedData('en-US', ['./test/testfiles/files3'], 'info');
            await parsedDataCache.getParsedData('en-US', ['./test/testfiles/files4'], 'info');
            ...
            const files3Data = parsedDataCache.getCachedData('./test/testfiles/files3', 'info', 'en');
            const files4Data = parsedDataCache.getCachedData('./test/testfiles/files4', 'info', 'en-US');
