...
import writeFiles, { writeHierarchicalFiles } from './write.js';
...
function mergeJson(options) {
    ...
    return import(pathToFileURL(assemblePath).href)
        .then(({ assemble, assembleHierarchical }) => {
            const result_data = isSplit
                ? assembleHierarchical([...ilibModules], options)
                : assemble([...ilibModules], options);
            isSplit
                ? writeHierarchicalFiles(result_data, outDir, isCompressed)
