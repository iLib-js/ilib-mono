/*
 * ResourceFullwidthLatinFixer.test.js - test the fixer for fullwidth Latin characters
 *
...
const rule = regexRules.find(rule => rule.name === "resource-no-fullwidth-latin");

describe("ResourceFullwidthLatinFixer", () => {
    test("fixes fullwidth Latin characters in simple string", () => {
        ...
        expect(resource.getTarget()).toBe("Boxにアップロード");
    });

    test("fixes fullwidth Latin characters in plural string", () => {
        const fixer = new ResourceFixer();
        const resource = new ResourcePlural({
            key: "test.key",
            source: {
                one: "Upload one file to Box",
                other: "Upload files to Box"
            },
            target: {
                other: "Ｂｏｘにファイルをアップロード"
            },
        });
        const source = getSource()
        const representation = getIntermediateRepresentation({ source, resource })
        ...
        expect(resource.getTarget().other).toBe("Boxにファイルをアップロード");
    });

    test("fixes fullwidth Latin characters in array string", () => {
        const fixer = new ResourceFixer();
        const resource = new ResourceArray({
            key: "test.key",
            source: ["Upload to Box", "Download from Box"],
            target: ["Ｂｏｘにアップロード", "Ｂｏｘからダウンロード"],
        });
        const source = getSource()
        const representation = getIntermediateRepresentation({ source, resource })
        ...
        expect(resource.getTarget()).toEqual(["Boxにアップロード", "Boxからダウンロード"]);
    });
});
