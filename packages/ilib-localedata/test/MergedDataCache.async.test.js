...
describe('MergedDataCache Async Tests (Node and Browser)', () => {
    ...
    describe('loadMergedData load data from stored data', () => {
        test('should merge data from stored data asynchronously', async () => {
            ...
            mergedDataCache.storeData(testData, "./test/testfiles/files3");
            ...
            expect(mergedDataCache.hasMergedData("root", ["./test/testfiles/files3"], "info")).toBe(true);
        ...
        test('should cache merged data after loading it asynchronously', async () => {
            ...
            mergedDataCache.storeData(testData, "./test/testfiles/files3");
            const result = await mergedDataCache.loadMergedData("en-US", ["./test/testfiles/files3"], "info");
            expect(result).toBeTruthy();
            expect(mergedDataCache.hasMergedData("en-US", ["./test/testfiles/files3"], "info")).toBe(true);
            expect(mergedDataCache.getMergedDataCount()).toBeGreaterThan(0);
            expect(result).toEqual({ "a": "b en-US", "c": "d root", "e": "f en-US" });

            const result2 = mergedDataCache.getCachedData("en-US", ["./test/testfiles/files3"], "info");
        ...
        test('should merge data from multiple sublocales asynchronously', async () => {
            ...
            expect(result).toBeTruthy();
            expect(mergedDataCache.hasMergedData("en", ["./test/testfiles/files3"], "info")).toBe(true);
            expect(mergedDataCache.getMergedDataCount()).toBeGreaterThan(0);
        ...
        test('should merge hierarchical data from stored data asynchronously', async () => {
            ...
            mergedDataCache.storeData(testData, "./test/testfiles/files2");
            ...
            expect(result).toBeTruthy();
            expect(mergedDataCache.hasMergedData("en", ["./test/testfiles/files2"], "tester")).toBe(true);
            expect(mergedDataCache.getMergedDataCount()).toBeGreaterThan(0);
    ...
    describe('loadMergedData', () => {
        ...
        test('should fall back to individual .json files when .js file not found', async () => {
            ...
            const result = await mergedDataCache.loadMergedData("en-US", ["./test/testfiles/files4"], "info");
        ...
        test('should return cached data if already loaded', async () => {
            ...
            const result1 = await mergedDataCache.loadMergedData("en-US", ["./test/testfiles/files3"], "info");
            expect(result1).toBeDefined();

            // Second call should return cached data
            const result2 = await mergedDataCache.loadMergedData("en-US", ["./test/testfiles/files3"], "info");
        ...
        test('should handle locale parameter as string or Locale object', async () => {
            ...
            const result1 = await mergedDataCache.loadMergedData("en-US", ["./test/testfiles/files3"], "info");
            const result2 = await mergedDataCache.loadMergedData(new Locale("en-US"), ["./test/testfiles/files3"], "info");
        ...
        test('should return undefined when no data can be loaded', async () => {
            ...
            const result = await mergedDataCache.loadMergedData("invalid", ["./test/testfiles/files3"], "info");
        ...
        test('should handle locale with only root data available', async () => {
            ...
            const result = await mergedDataCache.loadMergedData(null, ["./test/testfiles/files3"], "info");
        ...
        test('should merge multiple basenames correctly', async () => {
            ...
            const result = await mergedDataCache.loadMergedData("en-US", ["./test/testfiles/files3"], "foo");
        ...
        test('should handle locale parameter as Locale object directly', async () => {
            ...
            const result = await mergedDataCache.loadMergedData(locale, ["./test/testfiles/files3"], "info");
        ...
        test('should handle multiple roots with override precedence', async () => {
            ...
            const result = await mergedDataCache.loadMergedData("en-US", ["./test/testfiles/files4", "./test/testfiles/files3"], "info");
    ...
    describe('loadMergedData load data from files', () => {
        test('should load data from .js files asynchronously', async () => {
            ...
            expect(result).toBeTruthy();
            expect(mergedDataCache.hasMergedData("hu-HU", ["./test/testfiles/files7"], "address")).toBe(true);
            expect(mergedDataCache.getMergedDataCount()).toBeGreaterThan(0);
            expect(result).toEqual({
                "region": "Megye",
            ...
            const result2 = mergedDataCache.getCachedData("hu-HU", ["./test/testfiles/files7"], "address");
            expect(result2).toBeTruthy();
            expect(result2).toEqual(result);
        });
        ...
        test('should load data from flat .json files asynchronously', async () => {
            ...
            expect(result).toBeTruthy();
            expect(mergedDataCache.hasMergedData("hu-HU", ["./test/testfiles/files4"], "foo")).toBe(true);
            expect(mergedDataCache.getMergedDataCount()).toBeGreaterThan(0);
            expect(result).toEqual({
                "m": "n hu-HU",
                "o": "p hu-HU"
            ...
            const result2 = mergedDataCache.getCachedData("hu-HU", ["./test/testfiles/files4"], "foo");
            expect(result2).toBeTruthy();
            expect(result2).toEqual(result);
        });
        ...
        test('should load data from hierarchical .json files asynchronously', async () => {
            ...
            expect(result).toBeTruthy();
            expect(mergedDataCache.hasMergedData("ja-JP", ["./test/testfiles/files2"], "tester")).toBe(true);
            expect(mergedDataCache.getMergedDataCount()).toBeGreaterThan(0);
            expect(result).toEqual({
            ...
            const result2 = mergedDataCache.getCachedData("ja-JP", ["./test/testfiles/files2"], "tester");
            expect(result2).toBeTruthy();
            expect(result2).toEqual(result);
        });
    });
    ...
    describe('merge options', () => {
        test('should handle mostSpecific option', async () => {
            ...
            const result = await cache.loadMergedData("en-US", ["./test/testfiles/files3"], "info");
        ...
        test('should handle crossRoots option', async () => {
            ...
            const result = await cache.loadMergedData("de-DE", ["./test/testfiles/files3", "./test/testfiles/files4"], "foo");
        ...
        test('should share cached data between instances with same options', async () => {
            ...
            const result1 = await cache1.loadMergedData("en-US", ["./test/testfiles/files3"], "info");
            const result2 = await cache2.loadMergedData("en-US", ["./test/testfiles/files3"], "info");
    ...
    describe('loadMergedDataSync', () => {
        test('should return cached data if already available', async () => {
            ...
            await mergedDataCache.loadMergedData("en-US", ["./test/testfiles/files3"], "info");

            // Then retrieve it synchronously
            const result = mergedDataCache.loadMergedDataSync("en-US", ["./test/testfiles/files3"], "info");
    ...
    describe('getCachedData', () => {
        test('should return undefined when no data is cached', async () => {
            ...
            const result = mergedDataCache.getCachedData("en-US", ["./test/testfiles/files3"], "info");
            expect(result).toBeUndefined();
        });
        ...
        test('should handle invalid parameters gracefully', async () => {
            ...
            expect(mergedDataCache.getCachedData(null, ["./test/testfiles/files3"], "info")).toBeUndefined();
            ...
            expect(mergedDataCache.getCachedData({}, ["./test/testfiles/files3"], "info")).toBeUndefined();
        });
    });
    ...
    describe('hasMergedData', () => {
        test('should return true when data is cached', async () => {
            ...
            expect(mergedDataCache.hasMergedData("en-US", ["./test/testfiles/files3"], "info")).toBe(false);
    ...
    describe('getMergedDataCount', () => {
        ...
        test('should handle multiple cached entries', async () => {
            ...
            await mergedDataCache.loadMergedData("en-US", ["./test/testfiles/files3"], "info");
            await mergedDataCache.loadMergedData("en-GB", ["./test/testfiles/files3"], "info");
    ...
    describe('edge cases', () => {
        test('should handle empty locale data properly', async () => {
            ...
            try {
                await mergedDataCache.loadMergedData("", ["./test/testfiles/files3"], "info");
                fail("Expected loadMergedData to throw");
            } catch (e) {
        ...
        test('should handle undefined locale properly', async () => {
            ...
            try {
                await mergedDataCache.loadMergedData(undefined, ["./test/testfiles/files3"], "info");
                fail("Expected loadMergedData to throw");
            } catch (e) {
                expect(e).toBeDefined();
            }
    ...
    describe('loadLocaleData', () => {
        test('should load locale-specific data from .js files', async () => {
            ...
            const result = await mergedDataCache.loadLocaleData("en-US", ["./test/testfiles/files3"]);

            expect(result).toBe(true);
            // Verify that data was loaded for this locale and basename
            expect(mergedDataCache.hasLocaleData("en-US", ["./test/testfiles/files3"], "info")).toBe(true);
        });
        ...
        test('should load locale-specific data from .json files', async () => {
            ...
            const result = await mergedDataCache.loadLocaleData("ja-JP", ["./test/testfiles/files3"]);

            expect(result).toBe(true);
            // Verify that data was loaded for this locale and basename
            expect(mergedDataCache.hasLocaleData("ja-JP", ["./test/testfiles/files3"], "info")).toBe(true);
        });
        ...
        test('should return false when no locale-specific files are found', async () => {
            ...
            const result = await mergedDataCache.loadLocaleData("invalid-locale", ["./test/testfiles/files3"]);

            expect(result).toBe(false);
            // Verify that no data was loaded for this locale and basename
            expect(mergedDataCache.hasLocaleData("invalid-locale", ["./test/testfiles/files3"], "info")).toBe(false);
        });
        ...
        test('should handle multiple roots and return true if any data is found', async () => {
            ...
            const result = await mergedDataCache.loadLocaleData("en-US", ["./test/testfiles/files3", "./test/testfiles/files4"]);

            expect(result).toBe(true);
            // Verify that data was loaded for this locale and basename in at least one root
            expect(mergedDataCache.hasLocaleData("en-US", ["./test/testfiles/files3", "./test/testfiles/files4"], "info")).toBe(true);
        });

        test('should handle locale parameter as string or Locale object', async () => {
            expect.assertions(2);

            const result1 = await mergedDataCache.loadLocaleData("en-US", ["./test/testfiles/files3"]);
            const result2 = await mergedDataCache.loadLocaleData(new Locale("en-US"), ["./test/testfiles/files3"]);
        ...
        test('should cache data for multiple basenames from single locale file', async () => {
            ...
            expect(mergedDataCache.hasLocaleData("en-US", ["./test/testfiles/files3"], "info")).toBe(true);
            expect(mergedDataCache.hasLocaleData("en-US", ["./test/testfiles/files3"], "foo")).toBe(true);
        });
        ...
        test('should handle root locale correctly', async () => {
            ...
            const result = await mergedDataCache.loadLocaleData("root", ["./test/testfiles/files3"]);

            expect(result).toBe(true);
            // Verify that root data was loaded for this basename
            expect(mergedDataCache.hasLocaleData("root", ["./test/testfiles/files3"], "info")).toBe(true);
        });
        ...
        test('should handle complex locale with multiple sublocales', async () => {
            ...
            const result = await mergedDataCache.loadLocaleData("zh-Hans-CN", ["./test/testfiles/files3"]);

            expect(result).toBe(true);
            // Verify that data was loaded for this locale and basename
            expect(mergedDataCache.hasLocaleData("zh-Hans-CN", ["./test/testfiles/files3"], "info")).toBe(true);
        });
        ...
        test('should return cached result on subsequent calls', async () => {
            ...
            const result1 = await mergedDataCache.loadLocaleData("en-US", ["./test/testfiles/files3"]);
            const result2 = await mergedDataCache.loadLocaleData("en-US", ["./test/testfiles/files3"]);
        ...
        test('should handle invalid parameters gracefully', async () => {
            ...
            // Test with null locale
            try {
                await mergedDataCache.loadLocaleData(null, ["./test/testfiles/files3"]);
                fail("Expected loadLocaleData to throw");
            } catch (e) {
                expect(e).toBeDefined();
            }
            ...
            // Test with undefined roots
            try {
                await mergedDataCache.loadLocaleData("en-US", undefined);
                fail("Expected loadLocaleData to throw");
            } catch (e) {
                expect(e).toBeDefined();
            }
            ...
            // Test with empty roots array
            try {
                await mergedDataCache.loadLocaleData("en-US", []);
                fail("Expected loadLocaleData to throw");
            } catch (e) {
                expect(e).toBeDefined();
            }
            ...
            // Test with invalid locale object
            try {
                await mergedDataCache.loadLocaleData({}, ["./test/testfiles/files3"]);
                fail("Expected loadLocaleData to throw");
            } catch (e) {
                expect(e).toBeDefined();
            }
        ...
        test('should handle malformed locale files gracefully', async () => {
            ...
            const result = await mergedDataCache.loadLocaleData("invalid", ["./test/testfiles/files3"]);
            expect(result).toBe(false);
        });
    });
    ...
    describe('storeData', () => {
        test('should store data correctly', async () => {
            ...
            const cachedData = mergedDataCache.getCachedData("root", ["./test/testfiles/files3"], "info");
            ...
            expect(mergedDataCache.hasMergedData("root", ["./test/testfiles/files3"], "info")).toBe(false);
        ...
        test('should store multiple locales and basenames', async () => {
            ...
            expect(mergedDataCache.hasMergedData("root", ["./test/testfiles/files3"], "info")).toBe(false);
            expect(mergedDataCache.hasMergedData("en", ["./test/testfiles/files3"], "info")).toBe(false);
            expect(mergedDataCache.hasMergedData("root", ["./test/testfiles/files3"], "numbers")).toBe(false);
            expect(mergedDataCache.getMergedDataCount()).toBe(0);
        });
        ...
        test('should handle invalid data gracefully', async () => {
            ...
            // Test with null data
            mergedDataCache.storeData(null, "./test/testfiles/files3");
            expect(mergedDataCache.getMergedDataCount()).toBe(0);

            // Test with undefined data
            mergedDataCache.storeData(undefined, "./test/testfiles/files3");
            expect(mergedDataCache.getMergedDataCount()).toBe(0);
        });
