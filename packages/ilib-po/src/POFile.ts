...
import pluralForms from "./pluralforms.json";
...
class POFile {
    ...
    private commentsToIgnore: Set<string>;
                ...
                case State.START:
                    switch (token.type) {
                        ...
                        case TokenType.PLURAL:
                            var language = this.targetLocale.getLanguage();
                        ...
                        case TokenType.BLANKLINE:
                            if (source || sourcePlurals) {
                                ...
                                set.add(this.API.newResource(options));
                            }
    ...
    /**
     * Generate the PO file again from the resources.
    ...
    write(set: TranslationSet) {
        ...
        fs.writeFileSync(p, this.generate(set), "utf-8");
    }
}
