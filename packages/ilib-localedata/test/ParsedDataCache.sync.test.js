...
describe('ParsedDataCache Sync Tests (Node Only)', () => {
    ...
    describe('getParsedDataSync', () => {
        test('should load and cache data from .js files synchronously', () => {
            ...
            const roots = ["./test/testfiles/files7"];
            const basename = "info";
            ...
            expect(parsedDataCache.hasParsedData("./test/testfiles/files7", "info", null)).toBe(true);
            expect(parsedDataCache.hasParsedData("./test/testfiles/files7", "info", "en")).toBe(true);
            expect(parsedDataCache.hasParsedData("./test/testfiles/files7", "info", "en-US")).toBe(false);
        });
        ...
        test('should load and cache data from .js files with only root data synchronously', () => {
            ...
            expect(parsedDataCache.hasParsedData("./test/testfiles/files7", "info", null)).toBe(true);
            expect(parsedDataCache.hasParsedData("./test/testfiles/files7", "info", "root")).toBe(true);
        ...
        test('should load data from .js files for specific locale synchronously', () => {
            ...
            expect(result).toBeUndefined();
            expect(parsedDataCache.hasParsedData("./test/testfiles/files7", "info", "en")).toBe(false);
        ...
        test('should handle multiple roots and return data from the first available synchronously', () => {
            ...
            const locale = new Locale("en-US");
            const roots = ["./test/testfiles/files4", "./test/testfiles/files7"]; // First root doesn't exist
            const basename = "info";
        ...
        test('should return data from first available root synchronously', () => {
            ...
            const locale = new Locale("en-US");
            const roots = ["./test/testfiles/files4", "./test/testfiles/files7"]; // files4 has data, files7 doesn't
            const basename = "info";
    ...
    describe('Fallback from .js to .json files (files2)', () => {
        test('should fall back from .js files to hierarchical .json files when .js files have no data for locale', () => {
            ...
            const locale = new Locale("en-US");
            const roots = ["./test/testfiles/files7", "./test/testfiles/files2"]; // files7 has .js but no en-US data, files2 has hierarchical .json
            ...
            expect(parsedDataCache.hasParsedData("./test/testfiles/files7", "merge", "en-US")).toBe(false); // Should cache null for attempted load
        });
        ...
        test('should load hierarchical .json files with different basenames', () => {
            ...
            const locale = new Locale("en-US");
            const roots = ["./test/testfiles/files7", "./test/testfiles/files2"];
            ...
            expect(result).toBeTruthy();
            expect(parsedDataCache.hasParsedData("./test/testfiles/files2", "tester", "en-US")).toBe(true);
            expect(parsedDataCache.hasParsedData("./test/testfiles/files7", "tester", "en-US")).toBe(false); // Should cache null for attempted load
        });
        ...
        test('should automatically load data for all sublocales when loading .json files', () => {
            ...
            expect(parsedDataCache.hasParsedData("./test/testfiles/files2", "merge", null)).toBe(false);  // no root data
            ...
            const enResult = parsedDataCache.getCachedData("./test/testfiles/files2", "merge", "en");
        ...
        test('should automatically load data for the root locale when loading .json files', () => {
            ...
            expect(parsedDataCache.hasParsedData("./test/testfiles/files2", "merge2", null)).toBe(true);
            expect(parsedDataCache.hasParsedData("./test/testfiles/files2", "merge2", "fr")).toBe(false); // No fr data found
            expect(parsedDataCache.hasParsedData("./test/testfiles/files2", "merge2", "fr-FR")).toBe(false); // No fr-FR data found

            // Should be able to get the data for the root locale without loading anything else
            result = parsedDataCache.getCachedData("./test/testfiles/files2", "merge2", null);
            expect(result).toBeTruthy();
            expect(result).toEqual({ "a": "a from files2", "c": "c from files2", "d": "d from files2" });
            expect(parsedDataCache.hasParsedData("./test/testfiles/files2", "merge2", "fr-FR")).toBe(false); // Should cache null for attempted load
        });
        ...
        test('should handle complex locale with flat .json files', () => {
            ...
            expect(result).toBeTruthy();
            expect(parsedDataCache.hasParsedData("./test/testfiles/files7", "foo", "ja-JP")).toBe(true);

            // Check that intermediate locales are not cached because they aren't mentioned in the file
            expect(parsedDataCache.hasParsedData("./test/testfiles/files7", "foo", "ja")).toBe(false);
            expect(parsedDataCache.hasParsedData("./test/testfiles/files7", "foo", null)).toBe(false);
        });
    });
    ...
    test('should handle complex locale with hierarchical .json files', () => {
        ...
        expect(result).toBeTruthy();
        expect(parsedDataCache.hasParsedData("./test/testfiles/files2", "tester", "ja-JP")).toBe(true);

        // Check that intermediate locales are also cached, even though they do not exist on disk
        expect(parsedDataCache.hasParsedData("./test/testfiles/files2", "tester", "ja")).toBe(false);
        expect(parsedDataCache.hasParsedData("./test/testfiles/files2", "tester", null)).toBe(false);
    });
    ...
    describe('Parameter Validation', () => {
        test('should handle invalid parameters in getParsedDataSync', () => {
            ...
            const nullLocaleResult = parsedDataCache.getParsedDataSync(null, ['./test/testfiles/files2'], 'tester');
            expect(nullLocaleResult).toBeUndefined();

            // Test with undefined locale
            const undefinedLocaleResult = parsedDataCache.getParsedDataSync(undefined, ['./test/testfiles/files2'], 'tester');
            ...
            const nullBasenameResult = parsedDataCache.getParsedDataSync(new Locale('en-US'), ['./test/testfiles/files2'], null);
            expect(nullBasenameResult).toBeUndefined();

            // Test with undefined basename
            const undefinedBasenameResult = parsedDataCache.getParsedDataSync(new Locale('en-US'), ['./test/testfiles/files2'], undefined);
        ...
        test('should handle invalid parameters in hasParsedData', () => {
            ...
            // Test with null basename
            expect(parsedDataCache.hasParsedData('./test/testfiles/files2', null, 'en-US')).toBe(false);

            // Test with undefined basename
            expect(parsedDataCache.hasParsedData('./test/testfiles/files2', undefined, 'en-US')).toBe(false);

            // Test with empty string root
            expect(parsedDataCache.hasParsedData('', 'tester', 'en-US')).toBe(false);

            // Test with empty string basename
            expect(parsedDataCache.hasParsedData('./test/testfiles/files2', '', 'en-US')).toBe(false);
        });
        ...
        test('should handle invalid parameters in getCachedData', () => {
            ...
            // Test with null basename
            const nullBasenameResult = parsedDataCache.getCachedData('./test/testfiles/files2', null, 'en-US');
            expect(nullBasenameResult).toBeUndefined();

            // Test with undefined basename
            const undefinedBasenameResult = parsedDataCache.getCachedData('./test/testfiles/files2', undefined, 'en-US');
            expect(undefinedBasenameResult).toBeUndefined();
            ...
            // Test with empty string basename
            const emptyBasenameResult = parsedDataCache.getCachedData('./test/testfiles/files2', '', 'en-US');
    ...
    describe('Multi-root data loading', () => {
        test('should load data from all roots for a given locale', () => {
            ...
            const result = parsedDataCache.getParsedDataSync('de-DE', ['./test/testfiles/files7', './test/testfiles/files4'], 'foo');

            expect(result).toBeDefined();

            // Verify data was loaded from files7
            expect(parsedDataCache.hasParsedData('./test/testfiles/files7', 'foo', 'de-DE')).toBe(true);
            const files7Data = parsedDataCache.getCachedData('./test/testfiles/files7', 'foo', 'de-DE');
        ...
        test('should load data from all roots even when some roots have no data', () => {
            ...
            const result = parsedDataCache.getParsedDataSync('en-US', ['./test/testfiles/files7', './test/testfiles/files5'], 'info');
        ...
        test('should load root data from all roots', () => {
            ...
            const result = parsedDataCache.getParsedDataSync(null, ['./test/testfiles/files7', './test/testfiles/files4'], 'info');

            expect(result).toBeDefined();

            // Verify root data was loaded from files7
            expect(parsedDataCache.hasParsedData('./test/testfiles/files7', 'info', null)).toBe(true);
            const files7RootData = parsedDataCache.getCachedData('./test/testfiles/files7', 'info', null);
        ...
        test('should cache null for missing data in all roots', () => {
            ...
            const result = parsedDataCache.getParsedDataSync('non-existent', ['./test/testfiles/files7', './test/testfiles/files4'], 'non-existent');

            expect(result).toBeUndefined();

            // Verify that null was cached for both roots (indicating attempted load)
            expect(parsedDataCache.hasParsedData('./test/testfiles/files7', 'non-existent', 'non-existent')).toBe(false);
        ...
        test('should load different data formats from different roots', () => {
            ...
            const result = parsedDataCache.getParsedDataSync('en-US', ['./test/testfiles/files7', './test/testfiles/files4'], 'info');
            ...
            expect(parsedDataCache.hasParsedData('./test/testfiles/files7', 'info', 'en')).toBe(true);
            expect(parsedDataCache.hasParsedData('./test/testfiles/files7', 'info', null)).toBe(true);
            const jsData = parsedDataCache.getCachedData('./test/testfiles/files7', 'info', 'en');
        ...
        test('should maintain separate cache entries for each root', () => {
            ...
            parsedDataCache.getParsedDataSync('en-US', ['./test/testfiles/files7'], 'info');
            parsedDataCache.getParsedDataSync('en-US', ['./test/testfiles/files4'], 'info');
            ...
            expect(parsedDataCache.hasParsedData('./test/testfiles/files4', 'info', null)).toBe(false);

            // Verify they contain different data
            const files7Data = parsedDataCache.getCachedData('./test/testfiles/files7', 'info', 'en');
        ...
        test('should not load data from ESM modules because they cannot be loaded synchronously in Javascript', () => {
            ...
            const result = parsedDataCache.getParsedDataSync('en-US', ['./test/testfiles/files3'], 'info');
            expect(result).toBeUndefined();
            ...
            expect(parsedDataCache.hasParsedData('./test/testfiles/files3', 'info', 'en')).toBe(false);
        ...
        test('should handle invalid parameters in getParsedDataSync', () => {
            ...
            // Test with null locale
            const nullLocaleResult = parsedDataCache.getParsedDataSync(null, ['./test/testfiles/files3'], 'info');
            expect(nullLocaleResult).toBeDefined();

            // Test with undefined locale
            const undefinedLocaleResult = parsedDataCache.getParsedDataSync(undefined, ['./test/testfiles/files3'], 'info');
            expect(undefinedLocaleResult).toBeUndefined();
            ...
            // Test with null basename
            const nullBasenameResult = parsedDataCache.getParsedDataSync(new Locale('en-US'), ['./test/testfiles/files3'], null);
            expect(nullBasenameResult).toBeUndefined();

            // Test with undefined basename
            const undefinedBasenameResult = parsedDataCache.getParsedDataSync(new Locale('en-US'), ['./test/testfiles/files3'], undefined);
            expect(undefinedBasenameResult).toBeUndefined();
        });

        test('should return correct extensions for ESM modules', () => {
            expect.assertions(1);

            const extensions = parsedDataCache._getJsFileExtensions('./test/testfiles/files3/esm-test');

            // Should return ['.cjs', '.js'] for ESM modules
