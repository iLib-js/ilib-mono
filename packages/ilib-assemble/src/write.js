...
/**
 * Write locale data into a hierarchical directory structure.
 * Each sublocale key maps to its own file: "root" → root.json,
 * "en" → en.json, "en/US" → en/US.json, etc.
 *
 * @param {Record<string, object>} allData Sublocale-keyed data map.
 * @param {string} outDir Output directory path.
 * @param {boolean} isCompressed When true, write minified JSON.
 * @returns {void}
 */
export function writeHierarchicalFiles(allData, outDir, isCompressed) {
    if (!allData || typeof allData !== "object") return;

    Object.entries(allData).forEach(([sublocaleKey, data]) => {
        const fileName = sublocaleKey.replace(/\//g, "-") + ".json";
