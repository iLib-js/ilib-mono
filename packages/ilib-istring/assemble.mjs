...
function assemble(options) {
    ...
    const here = join(dirname(Path.fileUriToPath(import.meta.url)), "locale");

    options.locales.forEach(locale => {
        const locales = Utils.getSublocales(locale);
        const locFiles = Utils.getLocFiles(locale, "plurals.json").map(file => {
        ...
        locFiles.forEach((file, i) => {
            ...
            if (cache[file]) {
                locData[loc].plurals = cache[file];
            } else if (existsSync(file)) {
                const data = readFileSync(file, "utf-8");
                const json = JSON5.parse(data);
                locData[loc].plurals = json;
                cache[file] = json;
            }
        });
