...
describe('ParsedDataCache Sync Tests (Node Only)', () => {
    ...
    describe('getParsedDataSync', () => {
        test('should load and cache data from .js files synchronously', () => {
            ...
            expect(parsedDataCache.hasParsedData("./test/files7", "info", "en-US")).toBe(true);
        });
        ...
        test('should load data from .js files for specific locale synchronously', () => {
            ...
            expect(result).toBeUndefined();
            expect(parsedDataCache.hasParsedData("./test/files7", "info", "en")).toBe(true); // Should cache null for attempted load
        ...
        test('should return data from first available root synchronously', () => {
            ...
            // ParsedDataCache should return data from files4 since it has en-US data
            expect(result).toBeTruthy();
            expect(parsedDataCache.getParsedDataCount()).toBeGreaterThan(0); // Should cache some data
        });
    });
    ...
    describe('Fallback from .js to .json files (files2)', () => {
        test('should fall back from .js files to hierarchical .json files when .js files have no data for locale', () => {
            ...
            expect(parsedDataCache.hasParsedData("./test/files7", "merge", "en-US")).toBe(true); // Should cache null for attempted load
        });
        ...
        test('should load hierarchical .json files with different basenames', () => {
            ...
            expect(parsedDataCache.hasParsedData("./test/files7", "tester", "en-US")).toBe(true); // Should cache null for attempted load
        });
        ...
        test('should automatically load data for all sublocales when loading .json files', () => {
            ...
            expect(parsedDataCache.hasParsedData("./test/files2", "merge", null)).toBe(true);  // no root data
        ...
        test('should automatically load data for the root locale when loading .json files', () => {
            ...
            expect(parsedDataCache.hasParsedData("./test/files2", "merge2", "fr")).toBe(true);
            expect(parsedDataCache.hasParsedData("./test/files2", "merge2", "fr-FR")).toBe(true);
            ...
            expect(parsedDataCache.hasParsedData("./test/files2", "merge2", "fr-FR")).toBe(true); // Should cache the root data
        });
    ...
    test('should handle complex locale with hierarchical .json files', () => {
        ...
        expect(parsedDataCache.hasParsedData("./test/files2", "tester", "ja")).toBe(true);
        expect(parsedDataCache.hasParsedData("./test/files2", "tester", null)).toBe(true);
    });
});
