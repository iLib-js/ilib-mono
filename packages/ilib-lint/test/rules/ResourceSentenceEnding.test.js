...
describe("ResourceSentenceEnding rule", function() {
    ...
    describe("per-locale and per-punctuation disable config", () => {
        ...
        test("constructs without throwing when a mixed locale object uses period false", () => {
            expect.assertions(1);
        ...
        test("ja-JP true keeps default Japanese period checking", () => {
            expect.assertions(2);
            ...
            expect(actual).toBeTruthy();
            expect(actual?.description).toContain('Sentence ending should be "。" (U+3002) for ja-JP locale instead of "." (U+002E)');
        });

        test("ja-JP period false skips missing or wrong kuten but still checks question marks", () => {
            expect.assertions(3);
            ...
            const missingPeriod = new ResourceString({
                key: "disable.ja.period.missing",
                sourceLocale: "en-US",
                source: "This is a sentence.",
                targetLocale: "ja-JP",
                target: "これは文です",
            ...
            expect(rule.matchString({
                source: missingPeriod.getSource(),
                target: missingPeriod.getTarget(),
                resource: missingPeriod,
                file: "a/b/c.xliff"
            })).toBeUndefined();

            const wrongPeriod = new ResourceString({
                key: "disable.ja.period.wrong",
                sourceLocale: "en-US",
                source: "This is a sentence.",
                targetLocale: "ja-JP",
                target: "これは文です.",
            ...
            expect(rule.matchString({
                source: wrongPeriod.getSource(),
                target: wrongPeriod.getTarget(),
                resource: wrongPeriod,
                file: "a/b/c.xliff"
            })).toBeUndefined();

            const wrongQuestion = new ResourceString({
                key: "disable.ja.period.question.still.on",
                sourceLocale: "en-US",
                source: "What is this?",
                targetLocale: "ja-JP",
                target: "これは何ですか?",
            ...
            const actual = rule.matchString({
                source: wrongQuestion.getSource(),
                target: wrongQuestion.getTarget(),
                resource: wrongQuestion,
                file: "a/b/c.xliff"
            });
            expect(actual?.description).toContain('Sentence ending should be "？" (U+FF1F) for ja-JP locale instead of "?" (U+003F)');
        });

        test("ja-JP period null still disables period checking", () => {
            expect.assertions(1);
        ...
        test("zh-CN period false skips Chinese period checks", () => {
            expect.assertions(1);
